...
class engagementAnalysis:
  def __init__(self, df):
    self.df_eng = df
  def get_user_related_columns(self):
    aggrigate = {
    ...
    user_app_usage = self.df_eng.copy()
    ...
    self.user_behaviour = user_app_usage.groupby('MSISDN/Number').agg(aggrigate)
    self.user_behaviour = utils.fix_outlier(self.user_behaviour, columns)
    return self.user_behaviour

  def bivariant(self,user_behaviour):
    fig, ((ax1, ax2, ax3, ax4), (ax5, ax6, ax7, ax8)) = plt.subplots(2, 4,figsize=(15,8))
    plot.plot_scatter(user_behaviour.sample(1000), "Total", "Total Social Media", "Total Vs Social Media",ax1, "", "")

  def bivariant_sactter(df, x_col, y_col, ax)->None:
    sns.scatterplot(data = df, x=x_col, y=y_col, ax=ax)

    sample_df = self.user_behaviour.sample(1000)
    bivariant_sactter(self.sample_df, 'Total', 'Total Social Media', ax1)
  ...
  def run_engagement():
        ...
        #f_clean = pd.read_csv(file_name)
    st.write("## User Engagement Analysis")
    user_df = get_user_related_columns(self.df)
    st.write(user_df.head())
        #st.ploting.hist(user_df, 'Total Google', 'green')
    correlation = user_df.corr()
    plot.plot_heatmap(correlation, 'Correlation B/n  Applications')
    st.write("## Univariant")
    univriant(user_df)
    st.write("## Bivariant")
    bivariant(user_df)
